...
HTMLHelpParser::HTMLHelpParser(const RVNGInputStreamPtr_t &input, librevenge::RVNGTextInterface *document)
  : m_input(input)
  , m_document(document)
{
  assert(m_input);
  assert(m_input->isStructured());
}

void HTMLHelpParser::parse()
{
  if (!m_document)
    return;
}
